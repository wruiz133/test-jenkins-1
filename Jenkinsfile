...
pipeline {
 ...
 stages {
        stage('Checkout-git'){
               steps{
		git poll: true, url: 'git@github.com:wruiz133/test-jenkins-1.git'
               }
        }
    ...
    stage('PushDockerImage') {
            steps {
             sh '''
                  docker tag apptest:latest wruiz170435/apptest:latest
					docker push wruiz170435/apptest:latest
